...
- [x] Add a public client portal (token-based read-only invoice link) — Batch 14: `migrations/0010_invoice_share.sql` adds `share_token UUID UNIQUE` + `share_expires_at` + a SECURITY DEFINER `crm_get_shared_invoice(p_token)` RPC (granted to anon) that returns one invoice + items + client name + issuer branding as redacted JSON. **No anon RLS policy on invoices** — the RPC takes the exact token so anon can't enumerate all shared invoices. Webview: `useShareInvoiceMutation` / `useRevokeShareMutation`; Share2 icon per row opens a dialog with expiry picker (24h/7d/30d/never), copyable URL, and revoke. Static portal page at `docs/portal/index.html` (GitHub Pages) reads `?p=<b64 url>&a=<b64 anon>&t=<token>`, calls the RPC via supabase-js CDN, renders a printable invoice. Single deploy serves all users since each share URL embeds the user's own project URL + anon key (anon keys are designed to be public).
